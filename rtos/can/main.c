...
init_status(void) {
        ...
        std_printf("CAN_FiR1(CAN1,0) = $%08X, R2(1) = $%08X\n",
                (unsigned)CAN_FiR1(CAN1,0),
                (unsigned)CAN_FiR2(CAN1,0));
        std_printf("CAN_FiR1(CAN1,2) = $%08X, R2(1) = $%08X\n",
                (unsigned)CAN_FiR1(CAN1,1),
                (unsigned)CAN_FiR2(CAN1,1));
}
...
//      rcc_periph_clock_enable(RCC_AFIO);
	...
	xTaskCreate(monitor_task,"monitor",300,NULL,configMAX_PRIORITIES-1,&montsk);
//	xTaskCreate(can_rx_task,"canrx",300,NULL,configMAX_PRIORITIES-1,&rxtsk);
